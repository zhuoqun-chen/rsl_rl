"""ConstrainedAMP_PPO: AMP_PPO + P3O cost constraints.

Composes CostConstraintMixin with AMP_PPO. Own update() that combines
AMP discriminator training with P3O cost penalty.
"""
...
class ConstrainedAMP_PPO(CostConstraintMixin, AMP_PPO):
  """AMP_PPO with P3O cost penalty."""
  ...
  def act(self, obs, amp_obs=None):
    actions = AMP_PPO.act(self, obs, amp_obs)
    if self.num_costs > 0:
  ...
  def process_env_step(self, obs, rewards, dones, extras, amp_obs=None, amp_obs_frames=None):
    ...
    AMP_PPO.process_env_step(self, obs, rewards, dones, extras, amp_obs, amp_obs_frames)

  def compute_returns(self, obs):
    AMP_PPO.compute_returns(self, obs)
    if self.num_costs > 0:
  ...
  def update(self):  # noqa: C901
    ...
    for sample, sample_amp_policy, sample_amp_expert in zip(generator, amp_policy_generator, amp_expert_generator):
      ...
      if self.num_costs > 0 and advantages_c_batch is not None:
        ...
        policy_loss += self.cost_value_loss_coef * cost_value_loss
